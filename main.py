...
# washing parameters
def compute_washing_parameters(type_of_dirt, degree_of_dirt, detergent_amount, clothes_amount):
    ...
    if 0 > detergent_amount < 100:
        raise Exception("Invalid Amount of detergent: %lf" % detergent_amount)
    if 0 > clothes_amount < 100:
        raise Exception("Invalid Amount of Clothes: %lf" % clothes_amount)

    type_fuzzy = fuzzify_laundry(type_of_dirt, degree_of_dirt, detergent_amount, clothes_amount)
...
def WriteScreen(
    ...
    print(f'The washing machine is {clothes_amount}% full with an amount of {detergent_amount} detergent, the clothes are {degree_of_dirt} points on degree of dirtiness and the type of dirt being {type_of_dirt}.\n')
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            detergent_amount = round(random.randint(0,99) + random.random(), 2)
            clothes_amount = round(random.randint(0,99) + random.random(), 2)
            washing_time = round(compute_washing_parameters(type_of_dirt, degree_of_dirt, detergent_amount, clothes_amount), 2)

            WriteScreen(type_of_dirt,degree_of_dirt,detergent_amount,clothes_amount,0)
            for i in range(int(washing_time)):
                remaining_time=int(washing_time)-i
                if clearCmd:
                    os.system(clearCmd)
                    WriteScreen(type_of_dirt,degree_of_dirt,detergent_amount,clothes_amount,remaining_time)
                else:
                    print(end='\r')
                    PrintRemainingTime(remaining_time)
                time.sleep(1)
